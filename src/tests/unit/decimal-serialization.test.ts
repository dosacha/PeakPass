...
/**
 * 화폐(unitPrice/totalAmount/totalPrice)는 직렬화 경계에서 string으로 통일한다.
 *
 ...
 *   2. GraphQLOrder.totalPrice의 *타입 선언*이 string인지 (컴파일 타임)
 *   3. GraphQL 스키마의 Order.totalPrice가 String! 인지 (런타임)
 */
describe('money serialization consistency (string-first)', () => {
  ...
  it('GraphQLOrder declares totalPrice as string', () => {
    ...
    expect(typeof sample.totalPrice).toBe('string');
  });

  it('GraphQL schema declares Order.totalPrice as String!', () => {
    ...
    const totalPriceField = fields.totalPrice;

    if (!totalPriceField) {
      throw new Error('Order.totalPrice field missing');
    }

    // String! 은 toString시 정확히 "String!" 으로 직렬화된다.
    expect(String(totalPriceField.type)).toBe('String!');
  });
});